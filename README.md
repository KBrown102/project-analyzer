...
所有逻辑在一个 `<script>` 里（约 3590 行），通过 `window.__PA` 暴露核心函数给测试。双击打开即用，无构建步骤，改完刷新见效果。
...
```
项目结构分析器-1.0.0/
├── project-analyzer.html     分析器本体，单文件零依赖（约 3590 行）
...
```bash
npm test          # smoke + regex + logic + scanner，758 项断言
...
- JSON 导出：序列化为合法 JSON，结构含 phases/artifacts/eng，可存档/分享/二次加工
...
这套测试用变异测试验过有效性：故意把判定分支、噪声正则、类型识别、`neg` 语义、优先级排序、`callAI` 降级、记忆容量、占位符注入、`parseAIJSON` strip、`buildDataSummary` 字段这几处分别改坏，全部被抓到。
...
| P2 | 只能一次性分析，无留存钩子 | 历史快照（localStorage）+ JSON 导出已上线 |
...
> 模板承载上限约 24 套（当前 18 套），超过后判定链不可维护。建议 22 套时启动数据驱动化重构。单文件约 3600 行已逼近拆分阈值，加 P3 AI 模块后增长加快，下次大改时考虑按域拆分。
